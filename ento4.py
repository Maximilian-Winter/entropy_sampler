...
class EntropyBasedSampler:
    ...
    def calculate_metrics(logits: torch.Tensor, attention_scores: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # Calculate attn_varentropy only if we have more than one element
        if attn_entropy.numel() > 1:
    ...
    def generate(self, input_ids: torch.Tensor, max_length: int, temperature: float = 0.666, top_p: float = 0.90,
                 top_k: int = 27, min_p: float = 0.1, use_static_cache: bool = False) -> List[int]:
        generator = torch.Generator(device=self.device).manual_seed(self.config['seed'])
        gen_tokens = input_ids.clone()

        # Initialize cache
        if use_static_cache:
            cache = StaticCache(self.config['n_layers'], self.config['max_seq_len'], self.config['n_heads'],
                                self.config['head_dim'])
        else:
            cache = DynamicCache()
        print("\n\n", flush=True)
        for _ in range(max_length - len(input_ids[0])):
            inputs = {'input_ids': gen_tokens[:, -1:] if _ > 0 else gen_tokens,
...
messages = [
    ...
    {"role": "user", "content": "What number is bigger 9.11 or 9.9? Take a deep relaxing breath and think step by step!"},
]
