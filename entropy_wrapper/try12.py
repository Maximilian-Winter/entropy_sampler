...
class EntropyAnalysisConfig:
    ...
    def __init__(self):
        ...
        self.hidden_state_analysis = ConfigurableAnalysis()  # New analysis
        self.kl_divergence = ConfigurableAnalysis()  # New analysis
...
class BaseEntropyAnalysisWrapper(ABC):
    ...
    def analyze_step(self, logits: torch.Tensor, attentions: List[torch.Tensor],
        ...
        Args:
            ...
            previous_logits: Logits tensor from the previous step.
        ...
        if self.config.logits_entropy.enabled:
            ...
            if self.config.surprisal.enabled:
                ...
                surprisal = -np.log(token_prob + 1e-12)
                step_analysis['surprisal'] = surprisal

            # KL divergence with previous step
            if previous_logits is not None and self.config.kl_divergence.enabled:
                prev_probs = F.softmax(previous_logits[:, -1, :], dim=-1)
                kl_div = F.kl_div(prev_probs.log(), probs, reduction='batchmean').item()
                step_analysis['kl_divergence'] = kl_div
        ...
        # Hidden state norms
        if self.config.hidden_state_analysis.enabled:
            # For the last token
            hidden_state_norms = [hidden_state[0, -1, :].norm().item() for hidden_state in hidden_states]
            step_analysis['hidden_state_norms'] = hidden_state_norms
    ...
    def visualize_kl_divergence(self, generation_results: Dict, folder_name: str):
        if not self.config.kl_divergence.enabled:
            logger.warning("KL Divergence visualization is not enabled in the configuration.")
            return

        steps = range(2, len(generation_results['step_analyses']) + 1)
        kl_divergences = [step['kl_divergence'] for step in generation_results['step_analyses'][1:]]

        plt.figure(figsize=(12, 6))
        plt.plot(steps, kl_divergences, label='KL Divergence', marker='o')
        plt.xlabel('Generation Step')
        plt.ylabel('KL Divergence')
        plt.title('KL Divergence Between Steps Over Time')
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(folder_name, 'kl_divergence_over_time.png'))
        plt.close()

    def visualize_hidden_state_norms(self, generation_results: Dict, folder_name: str):
        if not self.config.hidden_state_analysis.enabled:
            logger.warning("Hidden state norms visualization is not enabled in the configuration.")
            return

        steps = range(1, len(generation_results['step_analyses']) + 1)
        hidden_state_norms_per_step = [step['hidden_state_norms'] for step in generation_results['step_analyses']]

        # Transpose to get norms per layer
        hidden_state_norms_per_layer = list(zip(*hidden_state_norms_per_step))
        num_layers = len(hidden_state_norms_per_layer)

        plt.figure(figsize=(12, 6))
        for layer_idx, norms in enumerate(hidden_state_norms_per_layer):
            plt.plot(steps, norms, label=f'Layer {layer_idx}')
        plt.xlabel('Generation Step')
        plt.ylabel('Hidden State Norm')
        plt.title('Hidden State Norms Over Time')
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(folder_name, 'hidden_state_norms_over_time.png'))
        plt.close()
...
def save_metadata(wrapper: BaseEntropyAnalysisWrapper, config: EntropyAnalysisConfig, folder_name: str):
    metadata = {
        ...
        "hidden_state_analysis_enabled": config.hidden_state_analysis.enabled,
        "kl_divergence_enabled": config.kl_divergence.enabled,
...
def generate_html_report(generation_results: Dict, wrapper: BaseEntropyAnalysisWrapper, input_text: str,
                         folder_name: str, output_html_file: str):
    import os
    import glob
    ...
    input_ids = tokenizer(input_text, return_tensors='pt').input_ids.to(wrapper.device)
    input_length = input_ids.shape[1]

    # Get the generated portion ids
    generated_portion_ids = generated_ids[0][input_length:]

    # Get the tokens
    tokens = tokenizer.convert_ids_to_tokens(generated_portion_ids)
    ...
    for idx, token in enumerate(tokens):
        ...
        for key, value in step_analysis.items():
            ...
            value_str = str(value)
            # For lists, we can join them into a string
            if isinstance(value, list):
                value_str = ', '.join(map(str, value))
            # Sanitize the key to be used as data attribute
            data_key = key.replace('_', '-')
            data_attributes += f' data-{data_key}="{value_str}"'
        # Clean up the token for display
        display_token = token.replace('Ġ', ' ').replace('▁', '').strip()
        if display_token == '':
            display_token = token  # if token is empty after stripping, keep the original token
...
<h1>Visualizations</h1>
    ...
    for vis_file in visualization_files:
        vis_filename = os.path.basename(vis_file)
        visualization_images += f'<h2>{vis_filename}</h2><img src="{vis_filename}" alt="{vis_filename}" style="max-width: 100%;"><br>\n'

    html_page += visualization_images

    # Add JavaScript for tooltip
    html_page += """
<script>
const tooltip = document.getElementById('tooltip');
document.getElementById('generated-text').addEventListener('mousemove', function(e) {
    if (e.target.tagName.toLowerCase() === 'span') {
        let dataAttributes = e.target.dataset;
        let tooltipContent = '<ul>';
        for (let key in dataAttributes) {
            tooltipContent += '<li><strong>' + key + ':</strong> ' + dataAttributes[key] + '</li>';
        }
        tooltipContent += '</ul>';
...
if __name__ == "__main__":
    ...
    config.hidden_state_analysis.enabled = True
    config.kl_divergence.enabled = True
...
<|start_header_id|>assistant<|end_header_id|>"""
    ...
    for _ in range(10):
        ...
        wrapper.visualize_hidden_state_norms(generation_results, results_folder)
        wrapper.visualize_attention_entropies_per_head(generation_results, results_folder)
        wrapper.visualize_kl_divergence(generation_results, results_folder)
